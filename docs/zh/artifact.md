...
1. **Artifacts**：数据对象本身 - 包含二进制内容、元数据和版本信息
...
## 什么是 Artifact Service？

Artifact Service 是后端系统，负责：
